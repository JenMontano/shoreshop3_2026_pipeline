...
_LOCAL_UTILS = Path(__file__).resolve().parent / "utils"
if str(_LOCAL_UTILS) not in sys.path:
    sys.path.insert(0, str(_LOCAL_UTILS))
...
def _common_time_index(grid_files: list[Path]) -> xr.DataArray:
    """Union of time coordinates across grids.

    Cyclone compact files use integer timesteps (0..23). Calling
    ``pd.to_datetime`` on those ints turns them into ns-since-epoch
    datetimes, so a later ``reindex(time=...)`` matches nothing and
    fills the merge with NaNs. Preserve numeric times; only datetime-ify
    when the source coordinate is already datetime-like.
    """
    time_arrays: list[np.ndarray] = []
    for grid_file in grid_files:
        with xr.open_dataset(grid_file) as ds_temp:
            time_arrays.append(np.asarray(ds_temp.time.values))

    sample = time_arrays[0]
    if np.issubdtype(sample.dtype, np.datetime64):
        all_times: list[pd.Timestamp] = []
        for arr in time_arrays:
            all_times.extend(pd.to_datetime(arr).tolist())
        return xr.DataArray(pd.DatetimeIndex(sorted(set(all_times))), dims=["time"], name="time")

    all_vals = np.unique(np.concatenate([np.asarray(a).ravel() for a in time_arrays]))
    all_vals.sort()
    return xr.DataArray(all_vals, dims=["time"], name="time")
