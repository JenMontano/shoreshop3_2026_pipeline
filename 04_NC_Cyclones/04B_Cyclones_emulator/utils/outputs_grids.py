...
def merge_multiple_grids(grid_files, var_name='hs', steepness=10.0, tolerance_deg=0.001, output_file=None,
    ...
    for grid_idx, ds in enumerate(datasets):
        if 'coord_x' in ds.coords:
            lons = ds.coord_x.values
            lats = ds.coord_y.values
        else:
            lons = ds.lon.values
            lats = ds.lat.values
    ...
    # merged_data is a list of per-site arrays.
    # - For standard sea-state outputs, each entry has shape (n_times,)
    # - For cyclone outputs, each entry can have shape (n_cyclone, n_times)
    merged_data = np.array(merged_data)
    merged_coords = np.array(merged_coords)  # Shape: (n_sites, 2)
    
    # Get time (and optional cyclone) coordinates from first dataset
    first_var = datasets[0][var_name]
    has_cyclone_dim = "cyclone_id" in first_var.dims
    has_cyclone_coord_on_time = (
        ("cyclone_id" in datasets[0].coords)
        and ("time" in datasets[0]["cyclone_id"].dims)
    )
    times = datasets[0].time.values
    if has_cyclone_dim:
        cyclone_ids = datasets[0].coords["cyclone_id"].values
    elif has_cyclone_coord_on_time:
        cyclone_id_time = np.asarray(datasets[0]["cyclone_id"].values)
    ...
    # Create merged dataset, supporting both (time, site) and
    # (cyclone_id, time, site) layouts.
    attrs = {
        'long_name': datasets[0][var_name].attrs.get('long_name', var_name),
        'units': datasets[0][var_name].attrs.get('units', ''),
    }
    if not has_cyclone_dim:
        # merged_data shape: (n_sites, n_times) -> transpose to (time, site)
        data_var = (['time', 'site'], merged_data.T, attrs)
        coords = {
            'time': times,
            'site': np.arange(len(merged_data)),
        }
        # Preserve cyclone IDs when they are stored as a coordinate over time.
        if has_cyclone_coord_on_time:
            coords['cyclone_id'] = (['time'], cyclone_id_time)
    else:
        # merged_data shape: (n_sites, n_cyclone, n_times)
        # Reorder to (cyclone_id, time, site)
        merged_data_cts = np.moveaxis(merged_data, [0, 1, 2], [2, 0, 1])
        data_var = (['cyclone_id', 'time', 'site'], merged_data_cts, attrs)
        coords = {
            'cyclone_id': cyclone_ids,
            'time': times,
            'site': np.arange(merged_data_cts.shape[-1]),
        }
