"""GRID1 WHACS seapoint wind helpers (load, PCA prep, maps). No cropping — uses pre-cut files."""
...
GRID1_DIR = Path(__file__).resolve().parent.parent
INPUTS_DIR = GRID1_DIR / "inputs"
SWAN_INPUT = INPUTS_DIR / "CASES_ONLY_WIND/000/INPUT"
WIND_NC = {
    "uwnd": INPUTS_DIR / "uwnd_grid2.nc",
    "vwnd": INPUTS_DIR / "vwnd_grid.nc",
}
WIND_VALID_START = np.datetime64("1980-01-01T00:00:00")


def parse_swan_inpgrid(input_path: Path | None = None) -> dict[str, float]:
    """Parse the first INPGRID BOTTOM REGULAR line from a SWAN INPUT file."""
    input_path = Path(input_path or SWAN_INPUT)
    ...
    raise ValueError(f"No INPGRID BOTTOM REGULAR line in {input_path}")


SWAN_INPGRID = parse_swan_inpgrid()
...
def open_wind_dataset(
    ...
    """Merge GRID1 uwnd/vwnd WHACS seapoint files into one dataset for PCA."""
...
def plot_wind_file_input(
    ...
    *,
    ...
    depth_path = Path(depth_path or INPUTS_DIR / "CASES_ONLY_WIND/000/depth.dat")
