...
Quantitative predictions of coastal erosion and accretion rely critically on the quality of atmospheric and oceanic forcing. This repository implements the **wave-climate component** of the [ShoreShop3 benchmark](https://shoreshop3.netlify.app/) for the North Carolina coast (Montaño et al., 2020; Mao et al., 2025): computationally efficient, high-resolution hydrodynamic forcing for shoreline evolution models at scales from **hourly events** to **multi-decadal variability**, and from the **nearshore** to the **regional coastline** (up to **500 m** along reference transects).
...
- **Tropical cyclones:** Historical and emulated TC tracks expand plausible storm scenarios. A **TC track-type MLR** (03A) feeds a **SWAN parametric metamodel** (04A) that predicts cyclone wave fields; **04B** downscales spectra, partitions events (PTM4), and **splices** TC peaks into the non-TC century baselines from 03B.

Together, the framework delivers:

| Product | Period | Resolution | Folder |
|---------|--------|------------|--------|
| Hourly wave hindcast | 1980–2023 | Full domain + 500 m sites | `02_Wind_Metamodel/outputs/` |
...
Four linked stages. Each has a detailed README — use the sidebar or the [documentation map](#documentation-map) below.
...
```text
  ...
  ALR + GCM projections         → 04B spectra + splice
       │                              │
       └──────────┬───────────────────┘
                  ▼
...
|-------|-----------------|------------|--------------|
| [**01 — BinWaves**](01_BinWaves/README.md) | Nearshore swell hindcast | BinWaves / SWAN, kp reconstruction | `outputs/cropped_variables/`, `outputs/merged_grids/`, `gridN/outputs/kp_coeffs_*` |
| [**02 — Wind Metamodel**](02_Wind_Metamodel/README.md) | Wind-sea correction | KMA (250 BMUs) + smooth grid merge | `outputs/NorthCarolina/`, `outputs/merged_500m_binwaves_bmus/` |
| [**03 — Wave Emulator**](03_NC_Wave_Emulator/README.md) | Past & future wave climate | PCA + wave-cluster ALR; TC MLR (03A) | `03B/outputs/earth3_veg_ssp*/`, `03A/outputs/tc_logit/` |
| [**04 — Cyclones**](04_NC_Cyclones/README.md) | TC wave fields & splice | PCA/RBF metamodel (04A); BinWaves downscale + PTM4 (04B) | `04A/outputs/predicted_emu_tracks_*`, `04B/outputs/earth3_ssp*_with_cyclones/` |
...
Paths are **repo-relative**; **04B** reads upstream products via [`paths.py`](04_NC_Cyclones/04B_Cyclones_emulator/paths.py) (no copies in `04B/inputs/`).
...
```text
1. 01_BinWaves          propagation → reconstruction → partitions (per grid)
2. 02_Wind_Metamodel    01A full domain + 01B 500 m products
3. 03B                  partitions → KMA → PCA → ALR GCM notebooks
...
- **Bathymetry:** `01_BinWaves/inputs/gebco_bathymetry.nc` (referenced, not duplicated).
...
| [02 — Wind Metamodel](02_Wind_Metamodel/README.md) | KMA pipeline, BinWaves+BMUS merge, 500 m products |
...
> **Browse as a website:** `bash docs/build.sh` → open `docs/_build/html/intro.html` (see [docs/README.md](docs/README.md)).
...
- **Git:** track code and notebooks only; `outputs/`, `*.nc`, and large inputs are in [`.gitignore`](.gitignore).
